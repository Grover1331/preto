...
if($userID != "" || $language !="") {
	...
	if(empty($author_obj)) {
		...
		$myLang = array();
			$categories = get_the_terms( (int)$restID, 'language' );
			if($categories[0]->slug == $language) {
				$myLang[] = $restID;
			}
		if(empty($myLang)) {
			$json = array("success" => 0, "result" => 0, "error" => "Incorrect Restaurant");
		} else {
			$menuItems = array($restID);
			query_posts( array (  'post_type' => 'restaurants', 'post__in' => $menuItems) ); 
