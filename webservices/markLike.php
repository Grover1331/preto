...
if($userID != "" || $restID !="") {
	...
	if(empty($author_obj)) {
		...
		$countt = $wpdb->get_var("SELECT count(`restaurand_id`) FROM `wtw_likes` where `user_id`='$userID' AND `restaurand_id` = '$restID'");
		if($countt == 0) {
			$wpdb->insert( 'wtw_likes', array(
				'user_id' => $userID,
				'restaurand_id' => $restID)
			);
			$json = array("success"=> 1,"result"=> 1 ,"error"=>"No Error Found");
		} else {
			$wpdb->query("DELETE  FROM `wtw_likes` where `user_id`='$userID' AND `restaurand_id` = '$restID'");
